...
export default function BatchAnalysis() {
  ...
  return (
      ...
      <Panel className="ds-panel-pad">
        ...
        {inspecting && (
          <p style={{ marginTop: 14, fontSize: 13, color: 'rgb(var(--ds-muted))' }}>
            Reading the file…
          </p>
        )}
        ...
        {inspect && !inspect.fatal && (
          <div className="ds-fade-up" style={{ marginTop: 16 }}>
            ...
            <div style={{ display: 'grid', gap: 12, marginBottom: 15,
              ...
              <div>
                <div className="ds-mono" style={{ fontSize: 22 }}>{inspect.rows.toLocaleString()}</div>
                <div className="ds-section-label" style={{ marginTop: 4 }}>Rows</div>
              </div>
              <div>
                <div className="ds-mono" style={{ fontSize: 22 }}>{inspect.header.length}</div>
              ...
              <div>
                <div className="ds-mono" style={{ fontSize: 22 }}>
                  {(file.size / 1024).toFixed(0)}<span style={{ fontSize: 13 }}> KB</span>
                </div>
            ...
            <div style={{ display: 'grid', gap: 8, marginBottom: 14,
              ...
              {inspect.mapped.map((m) => (
                <div key={m.label} style={{ display: 'flex', gap: 8, alignItems: 'baseline',
                      fontSize: 12, padding: '7px 9px', borderRadius: 5,
                ...
                {inspect.findings.map((x, i) => {
                  ...
                  return (
                    <div key={i} style={{ borderRadius: 6, padding: '10px 12px',
                      ...
                      <div style={{ fontSize: 12, lineHeight: 1.6, marginTop: 5,
                                    color: 'rgb(var(--ds-muted))' }}>
                        {x.detail}
                      </div>
                      {x.rows?.length > 0 && (
                        <div className="ds-mono" style={{ fontSize: 11, marginTop: 6,
                  ...
                  {inspect.preview.map((r, i) => (
                    <tr key={i}>
                      {inspect.header.slice(0, 7).map((h) => (
                        <td key={h} className="ds-mono" style={{ fontSize: 12 }}>{r[h]}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
            ...
            <p style={{ fontSize: 11, color: 'rgb(var(--ds-faint))' }}>
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <label className="flex items-center gap-2 text-[14px] text-[rgb(var(--ds-muted))]">
            Alert at
            <input
              ...
              disabled={scoring}
              className="numeric w-24 rounded-md border px-2 py-1 text-[14px]"
              style={{ borderColor: 'rgb(var(--ds-line))',
                       background: 'rgb(var(--ds-surface))',
                       color: 'rgb(var(--ds-ink))' }}
            />
            <span className="text-[13px] text-[rgb(var(--ds-faint))]">
              {thresholdInput.trim() === ''
        ...
        <div style={{ marginTop: 18 }}>
          ...
          <SectionHeading
            ...
            action={<span className="ds-mono" style={{ fontSize: 11,
                    color: 'rgb(var(--ds-faint))' }}>
            ...
            <div>
              ...
              <ul style={{ margin: 0, padding: 0, listStyle: 'none',
                ...
                {[
                  ...
                  <li key={t} style={{ fontSize: 12, lineHeight: 1.55 }}>
          ...
          <p style={{ fontSize: 11, lineHeight: 1.6, color: 'rgb(var(--ds-faint))',
                      marginTop: 14 }}>
        ...
        <Panel className="ds-panel-pad">
          ...
          {scoring && latest && (
            <div className="ds-fade-up" style={{ display: 'flex', flexWrap: 'wrap', gap: 14,
              ...
              <span className="ds-mono" style={{ fontSize: 13, flex: '1 1 220px', minWidth: 0 }}>
                {latest.nameOrig} → {latest.nameDest}
              </span>
              <span style={{ fontSize: 12, color: 'rgb(var(--ds-muted))' }}>{latest.type}</span>
              <span className="ds-mono" style={{ fontSize: 13 }}>
                {Number(latest.amount).toLocaleString(undefined, { maximumFractionDigits: 0 })}
              </span>
              <span className="ds-mono" style={{ fontSize: 15, minWidth: 52, textAlign: 'right',
                    color: latest.alerted ? 'rgb(var(--ds-sev-critical))'
                      : 'rgb(var(--ds-accent-strong))' }}>
                {typeof latest.score === 'number' ? latest.score.toFixed(3) : '—'}
              </span>
              <span className="ds-mono" style={{ fontSize: 12, color: 'rgb(var(--ds-faint))' }}>
          ...
          {rows.length > 0 && (
            <div style={{ display: 'grid', gap: 12, marginTop: 14,
              ...
              {[
                ...
                <div key={label}>
                  <div className="ds-mono" style={{ fontSize: 22, color: colour }}>{value}</div>
            ...
            {narratives.map((n) => (
              <div key={n.index} style={{ background: 'rgb(var(--ds-workspace))',
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10,
                  ...
                  <span style={{ fontSize: 11, color: 'rgb(var(--ds-faint))',
                                 marginLeft: 'auto' }}>
                    cited, not invented
                  </span>
                </div>
                ...
                <thead className="sticky top-0 z-10 bg-sentinel-900">
                  <tr className="border-b border-subtle text-left">
                    {['Row', 'From → To', 'Type', 'Amount', 'Score', 'Verdict'].map((h) => (
                      <th
                        key={h}
                        className="px-4 py-2.5 text-[12px] font-semibold uppercase tracking-wider text-slate-500"
                      >
                        {h}
                      </th>
                    ))}
  ...
  return (
    ...
    >
      ...
      <td className="px-4 py-2.5">
        ...
        {row.typology_label && (
          <span className="ml-2 text-[12px] text-slate-600">{row.typology_label}</span>
        )}
      </td>
...
function Metric({ label, value, hint, emphasis }) {
  return (
    <div className={cx('rounded-xl border p-4', emphasis ? 'border-strong bg-surface-raised' : 'border-subtle bg-surface')}>
      ...
      <p className="mt-0.5 text-[12px] text-slate-600">{hint}</p>
    </div>
  )
}
