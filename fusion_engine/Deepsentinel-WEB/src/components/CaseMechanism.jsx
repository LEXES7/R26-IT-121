  ...
  return (
    <div className={cx('min-w-0', dim && 'opacity-55')}>
      <div className="flex items-baseline gap-2">
        ...
        <span className={cx('ml-auto text-[10px]', s.tone)}>{s.label}</span>
      </div>

      <p className="mt-2 text-[13px] font-medium text-slate-300">{lens.question}</p>
      <p className="mt-1.5 text-[11px] leading-relaxed text-slate-500">{lens.mechanism}</p>

      <div className="mt-4 min-h-[5rem]">{children}</div>

      <p className="hair-t mt-4 pt-2.5 text-[10px] leading-relaxed text-slate-600">
  ...
  return (
    <div>
      ...
      <div className="relative mt-5 h-9">
        ...
        >
          <span className="numeric absolute -top-1 left-1.5 whitespace-nowrap text-[10px] text-slate-500">
            {threshold.toFixed(2)}
          </span>
        </div>
        ...
        />
        <span
          className="numeric absolute top-[1.75rem] -translate-x-1/2 whitespace-nowrap text-[10px] text-slate-300"
      ...
      {d.out_of_training_distribution && (
        <p className="mt-2 text-[10px] leading-relaxed text-risk-medium">
      ...
      <div className="space-y-1.5">
        {counts.map((n, ring) => (
          <div key={ring} className="flex items-center gap-2.5">
            <span className="numeric w-14 shrink-0 text-[10px] text-slate-600">
            ...
            <span className="numeric ml-auto text-[10px] text-slate-500">{n}</span>
          </div>
        ))}
      </div>
      <p className="mt-2 text-[10px] leading-relaxed text-slate-600">
        ...
        <dl className="mt-4 flex flex-wrap gap-x-5 gap-y-1">
          {facts.map(([label, v]) => (
            <div key={label} className="flex items-baseline gap-1.5">
              <dd className="numeric text-[11px] text-slate-300">
  ...
  return (
    <div>
      ...
      {typeof predecessor?.attention_weight === 'number' && (
        <p className="numeric mt-1.5 text-[10px] text-slate-600">
          attention {predecessor.attention_weight.toFixed(3)}
        </p>
      )}
      {typeof evidence.step_burstiness === 'number' && (
        <p className="numeric mt-1 text-[10px] text-slate-600">
