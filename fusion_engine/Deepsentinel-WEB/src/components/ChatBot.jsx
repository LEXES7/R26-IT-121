      ...
      <div ref={scrollRef} className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
        ...
        {messages.length === 1 && suggestions.length > 0 && (
          <div className="space-y-1.5 pt-1">
            <p className="text-[13px] uppercase tracking-wide text-slate-500">Try asking</p>
      ...
      >
        ...
        {message.sources?.length > 0 && (
          <details className="mt-2 border-t border-subtle pt-2">
            <summary className="cursor-pointer text-[13px] text-slate-400 hover:text-slate-200">
              {message.sources.length} source{message.sources.length > 1 ? 's' : ''}
              {message.grounded === false && ' · quoted directly'}
            </summary>
            <ul className="mt-1.5 space-y-1">
              {message.sources.map((s) => (
                <li key={s} className="text-[13px] leading-snug text-slate-500">
                  {s}
                </li>
              ))}
            </ul>
