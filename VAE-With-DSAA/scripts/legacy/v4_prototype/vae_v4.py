"""
v4 VAE — PyTorch reimplementation matched to the v3 Keras specification.

TensorFlow has no Python 3.14 wheel, so the model is reimplemented in PyTorch.
Architecture, loss, Free Bits clamp, beta schedule, optimiser and early-stopping
rule are matched to v3. Glorot-uniform init reproduces the Keras default.

Protocols
    clean : chronological. scaler/VAE/kmeans fitted on the FIT partition only,
            threshold selected on the VAL partition, metrics on TEST only.
    leaky : reproduces the v3 protocol. scaler and VAE fitted on ALL normal
            rows, evaluation set contains those same rows, threshold tuned on
            a random 30% of the evaluation scores.
"""
...
# --------------------------------------------------------------------------
@torch.no_grad()
def encode_all(model: VAE, X: np.ndarray, bs: int = 16384):
    model.eval()
    mus, lvs, recons = [], [], []
